...
import { h, inputS, numS, Label, Field, Bar, Modal, ModalHead, Btn, Ghost, Danger, SectionLabel, MetricRow, Empty } from './ui.js';
...
function TxRow({ t, account, onEdit }) {
  ...
  return h('div', {
    onClick: () => onEdit(t.id),
    style: {
      display: 'grid', gridTemplateColumns: '1.6fr 1fr 0.8fr', gap: 20, alignItems: 'center',
    ...
    h('div', { style: { minWidth: 0 } },
      ...
      h('div', { style: { fontSize: 11.5, color: T.faint, marginTop: 4, ...NUM } }, t.date)
    ),
    h('div', { style: { fontSize: 12, color: T.muted, minWidth: 0, overflow: 'hidden', textOverflow: 'ellipsis', whiteSpace: 'nowrap' } },
  ...
  return h('div', null,
    ...
    h('div', { style: { display: 'grid', gridTemplateColumns: 'repeat(auto-fit, minmax(300px, 1fr))', gap: 56, alignItems: 'end' } },
      h('div', null,
        ...
        h('div', { style: { ...display(76, netColor), marginTop: 16 } },
    ...
    l.months.length > 1 && h('div', { style: { border: `1px solid ${T.border}`, marginTop: 56, background: T.card } },
      h(MonthChart, { byMonth: l.byMonth, months: l.months, monthly, onSetMonthly })
    ),

    h('div', { style: { display: 'grid', gridTemplateColumns: 'repeat(auto-fit, minmax(320px, 1fr))', gap: 56, marginTop: 56 } },
      h('div', null,
