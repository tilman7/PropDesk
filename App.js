...
import { h, useNarrow, Label, Field, Btn, Ghost, IconBtn, Mark, SectionLabel, Modal, ModalHead, Empty, numS, inputS } from './ui.js';
...
export default function App() {
  ...
  const goto = (v) => { setDetailId(null); setView(v); setMenuOpen(false); };
  ...
  const navItem = (id, label, badge) => h('button', {
    ...
    style: {
      ...
      borderRadius: 6, padding: narrow ? '13px 14px' : '7px 13px', fontSize: narrow ? 15 : 13.5,
      ...
      justifyContent: narrow ? 'space-between' : 'flex-start',
      textAlign: 'left',
    },
  }, h('span', null, label), badge || null);

  return h('div', {
    key: `${theme}-${currency}-${chfRate ? 1 : 0}`,
    style: { minHeight: '100vh', background: T.bg, color: T.text, fontFamily: SANS, overflowX: 'hidden', ...NUM },
  },
    h('style', null, `
      * { -webkit-tap-highlight-color: transparent; box-sizing: border-box; }
      html, body { margin: 0; max-width: 100%; overflow-x: hidden; overscroll-behavior-x: none; }
    ...
    h('header', { style: {
      ...
      h('div', { style: {
        maxWidth: 1160, margin: '0 auto', padding: narrow ? '0 16px' : '0 40px', height: narrow ? 56 : 62,
        display: 'flex', alignItems: 'center', justifyContent: 'space-between', gap: narrow ? 10 : 20,
      } },
        h('div', { style: { display: 'flex', alignItems: 'center', gap: narrow ? 0 : 30, minWidth: 0 } },
          h('button', {
            ...
            h(Mark, { size: narrow ? 17 : 19 }),
            h('span', { style: { fontSize: narrow ? 15 : 16, fontWeight: 600, letterSpacing: '-0.02em' } }, 'PropDesk'),
          ...
          !narrow && h('nav', { style: { display: 'flex', gap: 4 } },
            navItem('risk', 'Risiko'),
        ...
        h('div', { style: { display: 'flex', alignItems: 'center', gap: narrow ? 7 : 9 } },
          view === 'finances'
            ? h(React.Fragment, null,
                h(Ghost, {
                  onClick: () => setTxAddType('expense'),
                  style: { padding: narrow ? '8px 11px' : '8px 14px', fontSize: 13, whiteSpace: 'nowrap' },
                }, narrow ? '−' : '− Ausgabe'),
                h(Btn, {
                  onClick: () => setTxAddType('payout'),
                  style: { padding: narrow ? '9px 12px' : '9px 15px', fontSize: 13, background: T.green, borderColor: T.green, color: '#fff', whiteSpace: 'nowrap' },
                }, narrow ? '+' : '+ Payout'))
            : h(Btn, {
                onClick: () => setAdding(true),
                style: { padding: narrow ? '9px 12px' : '9px 15px', fontSize: 13, whiteSpace: 'nowrap' },
              }, narrow ? '+ Account' : 'Account anlegen'),
          narrow && h(IconBtn, {
            onClick: () => setMenuOpen((o) => !o),
            title: menuOpen ? 'Menü schliessen' : 'Menü',
          }, menuOpen ? '×' : '☰')
        )
      ),

      // Ausklappmenü: ersetzt auf schmalen Viewports die Navigationsleiste.
      narrow && menuOpen && h('div', { style: {
        borderTop: `1px solid ${T.border}`, background: T.headerBg,
        padding: '8px 10px 12px', display: 'flex', flexDirection: 'column', gap: 2,
      } },
        navItem('risk', 'Risiko'),
        navItem('finances', 'Finanzen', data.transactions.length > 0 && h('span', {
          style: { fontSize: 12.5, color: ledger.net >= 0 ? T.green : T.red, ...NUM },
        }, (ledger.net >= 0 ? '+' : '−') + fmt(Math.abs(ledger.net)))),
        navItem('archive', 'Archiv', archived.length > 0 && h('span', { style: { fontSize: 12.5, color: T.faint } }, archived.length)),
        navItem('settings', 'Einstellungen')
      )
    ),

    // Tap ausserhalb schliesst das Menü.
    narrow && menuOpen && h('div', {
      onClick: () => setMenuOpen(false),
      style: { position: 'fixed', inset: 0, zIndex: 15 },
    }),

    h('main', { style: { maxWidth: 1160, margin: '0 auto', padding: narrow ? '28px 16px 90px' : '52px 40px 110px' } },
      detail
        ? h(AccountDetail, {
