...
import { h, inputS, numS, Label, Field, Bar, Chip, PhaseTag, DataRow, Modal, ModalHead, Btn, Ghost, Danger, SectionLabel, ColHead, Panel } from './ui.js';
    ...
    h('div', { style: { display: 'grid', gridTemplateColumns: 'repeat(auto-fit, minmax(320px, 1fr))', gap: 44, marginTop: 44 } },
      ...
      h('div', null,
        ...
        h('div', { style: { display: 'grid', gridTemplateColumns: 'repeat(auto-fit, minmax(130px, 1fr))', gap: 14 } },
          liveField('Balance (live)', balance, setBalance, fmtAlt(parseFloat(balance) || 0, chfRate)),
          liveField('Höchster EOD-Stand', highWater, setHighWater),
          liveField('Bester Tag ($)', bestDay, setBestDay)
        ),
...
export function AccountForm({ initial, onClose, onSubmit, chfRate }) {
  ...
  return h(Modal, { onClose, max: 620, z: 60 },
    ...
    h('div', { style: two },
      ...
      h(Field, { label: `Max. Risiko pro Trade ($) · Standard ${fmt(riskDefault(f.phase))}` },
        h('input', { type: 'number', value: f.riskPerTrade, onChange: num('riskPerTrade'), style: { ...numS(), borderColor: T.text } })),
      h(Field, { label: 'Aktuelle Balance ($)' },
